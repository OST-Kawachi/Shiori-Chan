...
namespace ShioriChan.Services.MessagingApis.RichMenus {
	...
	public class RichMenuService : IRichMenuService {
		...
		/// リッチメニューを取得する
		/// </summary>
		public async Task<JToken> Get( string richMenuId ) {
		...
		/// </summary>
		/// <returns></returns>
		public async Task<Dictionary<string,string>> GetIds() {
			string channelAccessToken = this.oAuthRepository.GetNewlyChannelAccessToken();
			this.logger.LogTrace( $"Channel Access Token is {channelAccessToken}" );
			if( string.IsNullOrEmpty( channelAccessToken ) ) {
				ChannelAccessToken cat = this.channelAccessTokenService.Issue();
			...
			Dictionary<string , string> richMenuIds = new Dictionary<string , string>();
			try {
				...
				JArray richMenus = resultToken[ "richmenus" ] as JArray;
				foreach( JToken richMenu in richMenus ) {
					richMenuIds[ richMenu[ "name" ].ToString() ] = richMenu[ "richMenuId" ].ToString();
				}
